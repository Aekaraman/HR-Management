...
namespace HRManagement.Data.Repositories.Concrete
{
    ...
    public class PersonelRepository
    {
        ...
        public PersonelRepository()
        {
            ...
            client.BaseAddress = new Uri("http://localhost:61860/");
        }
        ...
        public IEnumerable<Personel> GetPersonelBirthdayByCompanyID(long companyID)
        {
            ...
            if (result.IsSuccessStatusCode)
            {
                var readTask = result.Content.ReadAsAsync<IList<Personel>>();//listeyi aldık
                readTask.Wait();
                personels = readTask.Result.Where(a => a.IsActive == true  && a.CompanyID==companyID).ToList();
                return personels;
            }
            else//boşsa liste çekilmediyse
                {
                personels = (List<Personel>)Enumerable.Empty<Personel>();//boş personel yolladık
                return personels;//boşu döndürdük
            }
        }


        public Personel GetPersonelLogin(string mail,string pass)
        {
            Personel personel = new Personel();
            ...
            if (result.IsSuccessStatusCode)
            {
                var readTask = result.Content.ReadAsAsync<IList<Personel>>();//listeyi aldık
                readTask.Wait();
                personel = readTask.Result.Where(a => a.Mail==mail && a.Password == pass).SingleOrDefault();
                return personel;
            }
            else//boşsa liste çekilmediyse
            {
                //boş personel yolladık
                return personel;//boşu döndürdük
            }
        }
